export function filterAndSpreadKeys(
	PageContent: { [x: string]: unknown },
	sectionsToFind: string[]
) {
	const sections = sectionsToFind.reduce((acc: { [key: string]: unknown }, section) => {
		acc[section] = Object.keys(PageContent).reduce(
			(sectionAcc: { [key: string]: unknown }, key) => {
